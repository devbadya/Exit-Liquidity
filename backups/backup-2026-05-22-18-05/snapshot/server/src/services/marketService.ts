...
export async function fetchMarketOverview(): Promise<MarketOverviewResponse> {
  const [globalJson, marketsJson, categoriesJson] = await Promise.all([
    fetchJson<{ data: Record<string, unknown> }>(CG_GLOBAL),
    fetchJson<CGMarket[]>(CG_MARKETS),
    fetchJson<{ name: string; market_cap: number; market_cap_change_24h: number; volume_24h: number; top_3_coins: string[] }[]>(
      CG_CATEGORIES,
    ).catch(() => []),
  ]);
