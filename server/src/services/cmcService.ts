...
/** Frische Top-Listings für Live-Ansicht (ein CMC-Request, max. 200 Coins) */
export async function fetchCmcLiveSnapshot(maxCoins = 200): Promise<CmcCoin[]> {
  const page = await fetchListingsPage(1);
  return page.slice(0, Math.min(maxCoins, page.length)).map(mapRow);
}
